...
if (
  !(
    redPixels.length === yellowPixels.length &&
    yellowPixels.length === greenPixels.length
  )
) {
  throw new Error(
...
const slices: SliceDefinition[] = redPixels
  ...
  .map(([x, y], index) => {
    ...
    const yellowPixel = yellowPixels.splice(nearestYellowPixelIndex, 1)[0];

    const nearestGreenPixelIndex = greenPixels.reduce(
      (acc, [greenX, greenY], greenIndex) => {
        if (greenX < yellowPixel[0] || greenY < yellowPixel[1]) {
          return acc;
        }
        const distance =
          Math.abs(greenX - yellowPixel[0]) + Math.abs(greenY - yellowPixel[1]);
    ...
    const moveHeight = yellowPixel[1] - start[1];
